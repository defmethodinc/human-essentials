...
RSpec.describe Distribution, type: :model do
  ...
  describe "validations" do
    ...
    it "validates shipping_cost is numerical and non-negative if shipped" do
    ...
    it "validates line_items_quantity_is_positive" do
      distribution.line_items.build(quantity: -1)
      expect(distribution).not_to be_valid
    end
  end
  ...
  describe "scopes" do
    ...
    it ".by_item_id filters by item_id" do
      item = create(:item)
      distribution.line_items.create(item: item, quantity: 5)
    ...
    it ".by_location filters by storage_location_id" do
    ...
    it ".recent returns recent distributions" do
      recent_distribution = create(:distribution, organization: organization, issued_at: Time.zone.now)
      expect(Distribution.recent).to include(recent_distribution)
    end

    it ".future returns future distributions" do
      future_distribution = create(:distribution, organization: organization, issued_at: Time.zone.tomorrow)
      expect(Distribution.future).to include(future_distribution)
    end

    it ".during filters distributions by date range" do
    ...
    it ".for_csv_export filters distributions for CSV export" do
  ...
  describe "instance methods" do
    describe "#distributed_at" do
      it "formats issued_at correctly" do
    ...
    describe "#copy_from_donation" do
      it "copies from a donation" do
        donation = create(:donation)
        distribution.copy_from_donation(donation.id, storage_location.id)
        expect(distribution.line_items.count).to eq(donation.line_items.count)
      end
    end

    describe "#initialize_request_items" do
      it "initializes request items" do
        request = create(:request, distribution: distribution)
        distribution.initialize_request_items
        expect(distribution.line_items.count).to eq(request.item_requests.count)
      end
    end

    describe "#copy_from_request" do
      it "copies from a request" do
    ...
    describe "#combine_distribution" do
      it "combines distribution line items" do
        expect(distribution.line_items).to receive(:combine!)
    ...
    describe "#csv_export_attributes" do
      it "returns CSV export attributes" do
    ...
    describe "#future?" do
      it "returns true if issued_at is in the future" do
        distribution.issued_at = Time.zone.tomorrow
    ...
    describe "#past?" do
      it "returns true if issued_at is in the past" do
        distribution.issued_at = Time.zone.yesterday
