...
RSpec.describe ProductDrive, type: :model do
  ...
  describe "scopes" do
    ...
    describe ".within_date_range" do
      it "filters by date range" do
        create(:product_drive, start_date: "2023-01-01", end_date: "2023-01-31")
        create(:product_drive, start_date: "2023-02-01", end_date: "2023-02-28")
        expect(ProductDrive.within_date_range("2023-01-01 - 2023-01-31").count).to eq(1)
      end
    end
  ...
  describe "instance methods" do
    ...
    describe "#distinct_items_count" do
      it "counts distinct items" do
        create(:line_item, itemizable: donation, item: create(:item))
    ...
    describe "#item_quantities_by_name_and_date" do
      it "returns item quantities filtered by date and sorted by name" do
        ...
        expect(product_drive.item_quantities_by_name_and_date("2023-01-01 - 2023-12-31")).to eq([10])
      end
    end

    describe "#donation_quantity_by_date" do
      it "calculates donation quantity by date and optional item category" do
        item_category = create(:item_category)
        item = create(:item, item_category: item_category)
        line_item.update(item: item, quantity: 10)
        expect(product_drive.donation_quantity_by_date("2023-01-01 - 2023-12-31", item_category.id)).to eq(10)
      end
    end

    describe "#distinct_items_count_by_date" do
      it "counts distinct items by date and optional item category" do
        item_category = create(:item_category)
        item = create(:item, item_category: item_category)
        line_item.update(item: item)
        expect(product_drive.distinct_items_count_by_date("2023-01-01 - 2023-12-31", item_category.id)).to eq(1)
      end
    end
  end
  ...
  describe "class methods" do
    describe ".search_date_range" do
      it "parses and returns search date range" do
