  ...
  describe "scopes" do
    describe ".by_name" do
      it "filters by name" do
        create(:product_drive, name: "Drive A", organization: organization)
        create(:product_drive, name: "Drive B", organization: organization)
    ...
    describe ".within_date_range" do
      it "filters by date range" do
        create(:product_drive, start_date: Date.today - 10, end_date: Date.today + 10, organization: organization)
        create(:product_drive, start_date: Date.today - 20, end_date: Date.today - 15, organization: organization)
        expect(ProductDrive.within_date_range("#{Date.today - 5} - #{Date.today + 5}").count).to eq(1)
      end
    end

    describe ".alphabetized" do
      it "orders by name" do
        drive_a = create(:product_drive, name: "Drive A", organization: organization)
        drive_b = create(:product_drive, name: "Drive B", organization: organization)
  ...
  describe "instance methods" do
    ...
    describe "#item_quantities_by_name_and_date" do
      it "returns item quantities filtered by date and sorted by name" do
        ...
        expect(product_drive.item_quantities_by_name_and_date(date_range)).to eq([5])
      end
    end
  ...
  describe "class methods" do
    describe ".search_date_range" do
      it "parses date range from string" do
        ...
        expect(ProductDrive.search_date_range(dates)).to eq({ start_date: "2023-01-01", end_date: "2023-12-31" })
      end
    end
  end
end
